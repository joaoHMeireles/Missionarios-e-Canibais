...
public class Missionario extends Personagem{
    ...
    public void colocar() {
        ...
        for(int i = 0; i < lado.size();i++){
            if(lado.get(i) instanceof Missionario){
                indice = i;
            }
        }

        if(indice == -1){
            throw new NaoTemEssaClasseException("missionário", " desse lado!");
        } else {
            if(Main.jangada[1] != null && Main.jangada[0] != null){
    ...
    public void tirar() {
        ...
        if(Main.jangada[0] == null && Main.jangada[1] == null){
            ...
            for(int i = 0; i < Main.jangada.length; i++){
                if(Main.jangada[i] instanceof Missionario){
                    indice = i;
                }
            }
            if(indice == -1){
                throw new NaoTemEssaClasseException("missionário", " na jangada");
            } else {
                Main.jangada[indice] = null;
                lado.add(new Missionario());
            }
        }
    }
}
